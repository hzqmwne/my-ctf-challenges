...
void GenerateTables(const char* hexKey, int Nk, int Nr, const char *hexbeforekey = nullptr, const char *hexafterkey = nullptr) {
  uint8_t key[Nk*4];
  ...
  uint8_t beforekey[16] = {};
  uint8_t afterkey[16] = {};
  if (hexbeforekey) {
    read_key(hexbeforekey, beforekey, 16);
  }
  if (hexafterkey) {
    read_key(hexafterkey, afterkey, 16);
  }
  for (unsigned i = 0; i < 4; i++) {
    roundKey[ 0*4 + i] ^= (uint32_t(beforekey[i*4+0])<<24) | (uint32_t(beforekey[i*4+1])<<16) | (uint32_t(beforekey[i*4+2])<<8) | beforekey[i*4+3];
  }
  for (unsigned i = 0; i < 4; i++) {
    roundKey[Nr*4 + i] ^= (uint32_t(afterkey[i*4+0])<<24) | (uint32_t(afterkey[i*4+1])<<16) | (uint32_t(afterkey[i*4+2])<<8) | afterkey[i*4+3];
  }
...
void syntax() {
  err_quit("Syntax: aes_whitebox_gen <aes128|aes192|aes256> <hex-key> [<beforekey> <afterkey>]");
}
...
int main(int argc, char* argv[]) {
  ...
  NTL::SetSeed((const unsigned char *)"0CTF2021", 8);

  if (argc == 5) {
    GenerateTables(argv[2], Nk, Nr, argv[3], argv[4]);
  } else {
